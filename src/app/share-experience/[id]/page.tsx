...
export default async function Home({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!experienceData) {
        return (
            <div className="min-h-screen bg-gray-900 flex items-center justify-center">
                <p className="text-white">⚠️ Experience not found.</p>
            </div>
        );
    }
    ...
    if (experienceData.user?.toString() !== userId && !isAdmin) {
        return (
            <div className="min-h-screen bg-gray-900 flex items-center justify-center">
                <p className="text-white">🚫 You do not have permission to edit this experience.</p>
            </div>
        );
    }

    const isAdminButUser = isAdmin && experienceData.user?.toString() === userId;
    ...
    return (
        <div className="min-h-screen bg-gray-900 overflow-x-hidden">
            ...
            <main className="container mx-auto px-4 py-4 sm:py-8">
                ...
                <InterviewStepper session={session} initialExperience={experienceData} isEdit={true} expId={id} isAdmin={isAdmin} isAdminButUser={isAdminButUser} />
            </main>
        </div>
    );
}
